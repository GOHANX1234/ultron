...
Ultron-3 is an open-source Android automation agent built with Flutter. It utilizes any OpenAI-compatible LLM API and native Android Accessibility Services to interpret screen layouts and execute multi-step tasks across any installed application via natural language commands.
...
Ultron-3 supports Android 8.0 (API 26) and newer. Current release builds are
...
4. Launch Ultron-3 and go to the **Settings** screen.
...
8. Enable the **"Ultron-3 Screen Control"** service in your Android Accessibility Settings.
...
1. Open **Settings → Apps → Ultron-3**.
2. Open the three-dot menu in the top-right corner.
3. Tap **Allow restricted settings** and confirm.
4. Return to Ultron-3 and open **Accessibility Settings** again.
5. Enable **Ultron-3 Screen Control**.

Ultron-3 now shows these instructions and provides shortcuts to both App
...
2. Input the token in the Ultron-3 Settings screen and enable the integration toggle.
